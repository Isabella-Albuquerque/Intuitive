...
export const dashboardService = {
  async getDados7Dias(usuarioId: number): Promise<DashboardData> {
    const chamadaSegura = async <T>(fn: () => Promise<T>, fallback: T) => {
      try {
        return await fn()
      } catch (err) {
    ...
    const [media, mediaFome, mediaSaciedade, distracao, emocoesAntes, emocoesDepois] = await Promise.all([
      chamadaSegura(() => this.getMedia7Dias(usuarioId), 0),
      chamadaSegura(() => this.getMediaFome7Dias(usuarioId), 0),
      chamadaSegura(() => this.getMediaSaciedade7Dias(usuarioId), 0),
      chamadaSegura(() => this.getDistracao7Dias(usuarioId), [{ countSim: 0, countNao: 0, mensagem: 'Sem dados' }]),
      chamadaSegura(() => this.getEmocoesAntes7Dias(usuarioId), [{}]),
      chamadaSegura(() => this.getEmocoesDepois7Dias(usuarioId), [{}]),
    ])
  ...
  async getDadosMensais(usuarioId: number): Promise<DashboardData> {
    ...
    const [media, mediaFome, mediaSaciedade, distracao, emocoesAntes, emocoesDepois] = await Promise.all([
      chamadaSegura(() => this.getMedia30Dias(usuarioId), 0),
      chamadaSegura(() => this.getMediaFome30Dias(usuarioId), 0),
      chamadaSegura(() => this.getMediaSaciedade30Dias(usuarioId), 0),
      chamadaSegura(() => this.getDistracao30Dias(usuarioId), [{ countSim: null, countNao: null, mensagem: 'Sem dados' }]),
      chamadaSegura(() => this.getEmocoesAntes30Dias(usuarioId), [{}]),
      chamadaSegura(() => this.getEmocoesDepois30Dias(usuarioId), [{}]),
    ])
  ...
  async getMedia7Dias(usuarioId: number): Promise<number> {
    const response = await api.get('/relatorios/mediarefeicoes/ultimos7dias', { params: { idUsuario: usuarioId } })
    return response.data || 0
  },

  async getMedia30Dias(usuarioId: number): Promise<number> {
    const response = await api.get('/relatorios/mediarefeicoes/ultimos30dias', { params: { idUsuario: usuarioId } })
    return response.data || 0
  },

  async getMediaFome7Dias(usuarioId: number): Promise<number> {
    const response = await api.get('/relatorios/media-fome/ultimos7dias', { params: { idUsuario: usuarioId } })
    return response.data || 0
  },

  async getMediaFome30Dias(usuarioId: number): Promise<number> {
    const response = await api.get('/relatorios/media-fome/ultimos30dias', { params: { idUsuario: usuarioId } })
    return response.data || 0
  },

  async getMediaSaciedade7Dias(usuarioId: number): Promise<number> {
    const response = await api.get('/relatorios/media-saciedade/ultimos7dias', { params: { idUsuario: usuarioId } })
    return response.data || 0
  },

  async getMediaSaciedade30Dias(usuarioId: number): Promise<number> {
    const response = await api.get('/relatorios/media-saciedade/ultimos30dias', { params: { idUsuario: usuarioId } })
    return response.data || 0
  },
  ...
  formatarDistracao(distracao: any) {
    if (!distracao || distracao.countSim == null || distracao.countNao == null) return null
    const total = distracao.countSim + distracao.countNao
    if (total === 0) return null

    return [
      { value: Math.round((distracao.countSim / total) * 100), label: 'Sim', color: '#E57373' },
      { value: Math.round((distracao.countNao / total) * 100), label: 'Não', color: '#3A7291' }
    ]
  },

  formatarEmocoes(emocoes: any) {
    const paleta = ['#2E6480', '#3A7291', '#4A83A2', '#5D94B3', '#73A6C4', '#8BB8D5', '#A5CAE6', '#C0DCF7', '#D6E8FF', '#E7F2FF', '#F5FAFF']
    const emojis = {
      countFeliz: '😊', countTriste: '😞', countCalmo: '😌', countAnsioso: '😰', countEstressado: '😣',
      countNeutro: '😐', countCulpado: '😔', countFrustrado: '😤', countCansado: '😴', countRelaxado: '😎', countEntediado: '🥱'
    }
    ...
    return emocoesPresentes.map(([key, value], index) => {
      ...
      return { value, label: emojis[key as keyof typeof emojis] || '😐', color: paleta[paletaIndex] }
    })
  }
}